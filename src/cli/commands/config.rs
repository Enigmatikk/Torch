...
fn cache_config() -> Result<(), Box<dyn std::error::Error>> {
    println!("{} Caching configuration...", "💾".yellow());
...
"#;
    ...
    println!("{} Configuration cached successfully", "✅".green());
    println!("{} Cache file: src/cache/config.rs", "📁".blue());
...
fn clear_config_cache() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if Path::new("src/cache/config.rs").exists() {
        ...
        println!("{} Configuration cache cleared successfully", "✅".green());
    } else {
...
fn show_config(key: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
    println!("{} Application Configuration", "⚙️".yellow().bold());
    println!();
    ...
    if let Some(search_key) = key {
        println!("{} Filtering by key: {}", "🔍".blue(), search_key.cyan());
        println!();
        ...
        if filtered.is_empty() {
            println!("{} No configuration found for key: {}", "❌".red(), search_key);
            return Ok(());
        }
